...
// 26-09-05（CX-260905-02 ②，綾實測）：ratio 的**分母不穩**。她的系統區在 19,214／14,657 兩個
// 尺寸之間規律擺盪（position=0 的世界書合併區塊每隔一發吞吐一次），系統區脹縮擠壓留給對話流的
// 空間 → 酒館塞進來的對話則數跟著在 17／21 之間跳。於是同一段對話，只因為上一發剛好塞得多，
// ratio 就從 0.588 掉到 0.476、基準被丟掉。連續吻合的**絕對值**不受分母影響：
// run ≥ 8 就直接算同一個對話（與 ratio 是 OR）。安全邊際有實測背書——同一張卡、同一句開場白的
// 兩個**不同**聊天 run=1，換卡 run=0（見 window-slide-conversation ②⑪ 那張表），距離 8 很遠。
export const CONV_OVERLAP_STRONG_RUN = 8;    // 連續吻合 ≥ 這麼多則，不看比率也算同一個對話
// 26-09-05 實彈照出來的：舊值 12 是照「實測位移多在 1–4 格」訂的，但綾當天就量到 7，
// 而承曦拿 15 格位移打真實請求時 run 直接是 0——**超出搜尋半徑就完全看不見**，
// 於是同一個對話被判成換對話、基準丟掉。單元測試全在 12 以內，照不出這一格。
// 放寬到全範圍（512 只是防呆上限）不會增加誤判：換對話的指紋在任何偏移都對不上（run=0），
// 同卡不同聊天最多 run=1，離 STRONG_RUN=8 與 ratio 門檻都很遠；成本是 O(偏移×則數) 的字串比對，可忽略。
export const CONV_OVERLAP_MAX_OFFSET = 512;  // 偏移搜尋半徑上限（實務不設限，純防呆）
