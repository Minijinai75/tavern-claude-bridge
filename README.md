...
橋跑起來後，`plugins\tavern-claude-bridge\` 裡會陸續長出幾個 `.jsonl` 檔（每行一筆 JSON 的純文字），都是給診斷用的。先講結論：**三個檔都可以隨時刪**，刪了橋照跑、只是那段紀錄沒了；每個檔超過 2MB 會自動砍掉前面、只留最後一段，不會無限長大。

- `cache-log.jsonl`：每送一則記一筆——模型、token 數、快取讀了多少寫了多少、系統提示的雜湊值。**只有數字與雜湊，不含對話內容**——唯一的例外是 `rewriteDiff` 欄：當橋偵測到某一則被改寫，會留一小段差異片段（改寫前後各 24 字＋前後文 36 字，合計最多 84 字），方便認出在變的是時間戳還是狀態欄。
- `turn-log.jsonl`（v1.8.7 起預設開）：每送一則記一筆，內容是**每一則的結構**——第幾則、是誰說的（角色）、多長、跟上一發比是同／移位／新，用來查「拆塊到底有沒有作用」。不含對話內容；只有你自己設了 `TCB_TURN_TRACE=1`（見下一節）才會多記每則開頭 30 字與被改寫那則的前後全文。
- `empty-replies.jsonl`：只在出現空白回覆時記一筆，內容是診斷欄位（stop_reason、subtype、成因判定、花費）與 SDK 回報的那段結果文字。不含任何一句對話。它存在的理由是：額度用盡那一刻沒人有辦法抄終端機，讓橋自己留證據、事後撈得到。

回報問題時，把這幾個檔一起附上，比截圖有用得多。
...
**省更多快取（試驗中，預設關）**
打勾後，角色卡跟系統提示會改放進對話的第一則一起送出。這樣橋可以多釘一個快取點：長對話裡「已經穩定的那一大段」跟「這次新增的那一段」各自標記，斷點往後跳的時候舊的那段照樣命中、只重寫新增的部分，省下來的錢會明顯多一截。代價：模型看角色卡的方式可能有一點不同，語氣可能微妙地變。想試就開、覺得怪就關，隨時可以切。實測（26-09-02，Opus 4.6）：開著打三發長對話，第三發命中 74%。

**診斷模式（平常不用開）**
打勾後，每一則送出去的內容開頭 30 個字會寫進你電腦裡的 `turn-log.jsonl`，方便對照是哪一則害快取失效。只存在你自己的電腦，不會傳給任何人。查完記得關掉。

另外兩件 1.9.0 改的、你不用動手但會感覺到的事：
- 系統提示邊界判斷修正：之前有一種形狀（設定用 user 角色寫、玩家先發言、世界書條目用深度插入）會把玩家的第一句話當成系統提示送出去、而且每次條目位移整包快取重建。現在不會了。
- 快取的份量門檻改算「到斷點為止的整個前綴」（Anthropic 實際上就是這樣算的）。之前只算對話那一段自己，大角色卡＋短對話會等很久才開始拆塊；現在早很多。

## 進階環境變數

一般使用者不用碰這一節。這些是啟動 SillyTavern **之前**設的環境變數（PowerShell 例：`$env:TCB_SPLIT='0'; node server.js`），每個一行講人話：

- `TCB_SPLIT=0`：從啟動層**鎖死**「拆塊省快取」，面板那格會變成不可點。用來排除「是不是拆塊害的」。
- `TCB_PERSIST_SESSION=0`：退回不保留 SDK session 的舊行為。對話流那一層的快取會吃不到（實測差 88%），只有很在意本機留 session 檔的人才需要。
- `TCB_TURN_TRACE=1`：**診斷模式**。turn-log 會多記每則開頭 30 字與被改寫那則的前後全文，終端機也多印一行斷點量測。**會把對話片段寫到磁碟**，查完記得關。
- `TCB_CACHE_BOUNDARY=1`：在系統提示尾端加 SDK 的動態邊界標記。實驗開關，**沒在有效條件下驗證過**，預設關。
